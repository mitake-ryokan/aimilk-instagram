...
def _postable(e):
    ...
    そこで「情報源が信頼できるかどうか」で分ける。
      ×                     → 投稿しない（人が明示的に止めた。これだけが「出さない」）
      それ以外               → 投稿する

    ■ 「確度が中・低」も出すようにした（★2026-08-20 変更）
    以前はここで止めていた。だが箱根では、地域の行事ほど日付が出回らない。
    止め続けると、いちばん載せたい行事が永久に載らない。
    → 出す。ただし日付は「10月中旬ごろ」とぼかし、
      AIみるくが「知ってる人は教えてにゃ」と聞く。詳しくは is_uncertain を参照。
    ...
    """
    return e.get("掲載可否", "").strip() != "×"


UNCERTAIN_LEVELS = ("中", "低")


def is_uncertain(e):
    """日付を確かめきれていない行か。

    ■ なぜ「出さない」から「ぼかして出す」に変えたのか（2026-08-20）
    箱根では、地域の行事の日付が直前まで出回らないことが珍しくない。
    「裏が取れないものは出さない」を貫くと、
    **いちばん地元らしい行事ほど載らない**という逆のことが起きる。
    かといって 10/15 と断定して出せば、違う日に人を行かせてしまう。

    そこで第三の道をとる。「10月中旬ごろ」とだけ書き、
    AIみるくが「知ってる人は教えてにゃ」と言う。
    断定しないので誤情報にならず、しかも
    **知っている人が教えてくれる入口になる。**
    確認の手間をこちらで抱え込まず、地域に開く。

    3号さんが日付を確かめたら、シートの確度を「高」にする。
    それだけで、次からは普通の書き方に戻る。
    """
    return e.get("確度", "").strip() in UNCERTAIN_LEVELS
...
def pick_week(events, start: dt.date, end: dt.date):
    ...
    # 日付があいまいな行は、同じ箱の中で後ろに回す。
    # 確かな情報を先に見せて、あいまいなものはその後ろで「教えてにゃ」と聞く順番。
    for bucket in (now, promo, long_):
        bucket.sort(key=lambda x: (is_uncertain(x[1]), x[0]))
